...
struct AppStoreReceiptValidator {
    ...
    static func validate(_ receiptData: Data, completion: @escaping (AppStoreValidationResult) -> Void = {_ in}) {
        ...
        var request = URLRequest(url: URL(string: "https://luckypunk.azurewebsites.net/api/ValidateAppStoreReceipt?code=RfK3TV9TTQKWwlde4kBaaaujdW8PlXTqT/A/4jsQXd2/oAlt5r4wAQ==")!)
        //and configure it to communicat. with the server side function URL
        */
        
        //prepare an http request
        var request = URLRequest(url: URL(string: "https://luckypunk.azurewebsites.net/api/HttpTrigger1?code=EwRC65Svoy24IKP4wc4iz1CZtPGPL7sNRspj1UyLRolglDMU33Benw==")!)
        //and configure it to communicat. with the server side function URL
